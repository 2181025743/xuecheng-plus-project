...
public class CourseBaseInfoServiceImpl implements CourseBaseInfoService {
    ...
    public CourseBaseInfoDto createCourse(Long companyId, AddCourseDto addCourseDto) {
        ...
        // ========== 第1步：参数合法性校验 ==========
        // Service层的校验是必须的！即使Controller层校验了也要校验
        // 原因：Service方法可能被其他方式调用（如RPC），不一定经过Controller
        ...
        if (StringUtils.isEmpty(addCourseDto.getMt())) {
            XuechengPlusException.cast("课程大分类不能为空");
        }
        if (StringUtils.isEmpty(addCourseDto.getSt())) {
            XuechengPlusException.cast("课程小分类不能为空");
        }
        ...
        if (StringUtils.isEmpty(addCourseDto.getTeachmode())) {
            XuechengPlusException.cast("教学模式不能为空");
        }
